...
TEST(ThreadedTree, InorderThreading)
{
    ...
    tree.traverseThreaded([&result](int val)
...
TEST(ThreadedTree, PreorderThreading)
{
    ...
    // Verify results are in correct preorder
...
TEST(ThreadedTree, PostorderThreading)
{
    ...
    // The correct postorder traversal for a BST with sequential insertion of 1-7
...
TEST(ThreadedTree, EmptyTree)
{
    ...
    int count = 0;
    tree.traverseThreaded([&count](int)
                          { count++; });
    EXPECT_EQ(count, 0);
}
